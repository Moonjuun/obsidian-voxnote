...
const README_PATH = 'VoxNote/README.md';
const FEATURES_PATH = 'VoxNote/FEATURES.md';
...
const KO_README = `# VoxNote — Meeting Transcription & AI Summary

이 폴더는 [VoxNote](https://github.com/Moonjuun/obsidian-voxnote) 플러그인이 자동 생성한 작업 공간입니다. **사용 중 자주 보는 기능 가이드는 같은 폴더의 [[FEATURES]] 파일에 모아두었습니다.**
...
![Deepgram 회원가입 화면](https://raw.githubusercontent.com/Moonjuun/obsidian-voxnote/main/img/img1.webp)
...
![API 키 생성 화면](https://raw.githubusercontent.com/Moonjuun/obsidian-voxnote/main/img/img2.webp)

### 2단계 — 플러그인에 API 키 입력

1. 옵시디언 → **설정** → **Community plugins** → "VoxNote" 옆 **톱니바퀴** 클릭.
2. **"Deepgram API 키"** 칸에 복사한 키 붙여넣기.
3. **"검증"** 버튼 클릭 → \`✓ API 키 유효\` 메시지 확인.

![플러그인 설정 — Deepgram 키 (필수) + Gemini 키 (AI 요약용, 선택)](https://raw.githubusercontent.com/Moonjuun/obsidian-voxnote/main/img/img3.webp)
...
1. **녹음 파일을 \`VoxNote/Audio/\`에 넣기**
   - **옵시디언 창에 직접 드래그앤드롭**: Finder에서 녹음 파일을 잡아 **옵시디언 창 좌측 사이드바**의 \`VoxNote/Audio/\` 폴더 위로 끌어다 놓기.
   - 또는 **Finder에서 직접 복사**: vault 폴더의 \`VoxNote/Audio/\` 안에 붙여넣기.

   > 💡 Finder에서 직접 복사한 경우 옵시디언 좌측 사이드바에 파일이 바로 안 보일 수 있습니다. 그 때는 \`Cmd+P\` (Windows/Linux: \`Ctrl+P\`) → **"저장하지 않고 앱 새로고침"** 실행 후 다음 단계로.

2. **변환 실행** — 사이드바에서 그 오디오 파일을 **우클릭 → \`VoxNote ▸\`** 서브메뉴에서 원하는 항목 선택:
   - **\`STT만 추출\`** — STT 회의록만 생성 (Deepgram 키만 있으면 됨)
   - **\`⭐ STT + AI 요약: 회의록\`** — STT + AI 요약을 한 번에 (Gemini 키 설정 시 노출)
   - **\`AI 요약 ▸\`** — 즐겨찾기가 아닌 다른 템플릿(인터뷰·강의노트 등)

   기존 마크다운 노트를 우클릭하면 \`VoxNote ▸ ⭐ AI 요약: 회의록\`으로 **재요약**도 가능합니다.

![VoxNote 우클릭 서브메뉴 (예시: 마크다운 노트의 재요약 메뉴)](https://raw.githubusercontent.com/Moonjuun/obsidian-voxnote/main/img/img4.webp)
...
- **GitHub repo**: https://github.com/Moonjuun/obsidian-voxnote
- **이슈/제안**: [GitHub Issues](https://github.com/Moonjuun/obsidian-voxnote/issues)
`;

const EN_README = `# VoxNote — Meeting Transcription & AI Summary

This folder is the auto-created workspace for the [VoxNote](https://github.com/Moonjuun/obsidian-voxnote) plugin. **Day-to-day feature guides live in [[FEATURES]] in this same folder.**
...
![Deepgram signup screen](https://raw.githubusercontent.com/Moonjuun/obsidian-voxnote/main/img/img1.webp)
...
![Create API Key](https://raw.githubusercontent.com/Moonjuun/obsidian-voxnote/main/img/img2.webp)

### Step 2 — Paste the API key into the plugin

1. Obsidian → **Settings** → **Community plugins** → click the **gear icon** next to "VoxNote".
2. Paste the key into **"Deepgram API key"**.
3. Click **"Validate"** → you should see \`✓ API key is valid\`.

![Plugin settings — Deepgram key (required) + Gemini key (optional, for AI summary)](https://raw.githubusercontent.com/Moonjuun/obsidian-voxnote/main/img/img3.webp)
...
1. **Put the recording into \`VoxNote/Audio/\`**
   - **Drag-and-drop into the Obsidian window**: grab the file in Finder/Explorer and drop it onto the \`VoxNote/Audio/\` folder in **Obsidian's left sidebar**.
   - Or copy it directly into the \`VoxNote/Audio/\` directory in your vault folder.
...
2. **Run transcription** — in the sidebar, **right-click** the audio file → **\`VoxNote ▸\`** submenu and pick one:
   ...
   You can also right-click any existing markdown note → \`VoxNote ▸ ⭐ AI summary: Meeting\` to **re-summarize** it.

![VoxNote right-click submenu (example shows the re-summarize menu on a markdown note)](https://raw.githubusercontent.com/Moonjuun/obsidian-voxnote/main/img/img4.webp)
...
- **GitHub repo**: https://github.com/Moonjuun/obsidian-voxnote
- **Issues / suggestions**: [GitHub Issues](https://github.com/Moonjuun/obsidian-voxnote/issues)
`;
...
![화자 이름 변경 명령](https://raw.githubusercontent.com/Moonjuun/obsidian-voxnote/main/img/img5.webp)
...
| \`{{audio_link}}\` | \`[[VoxNote/Audio/녹음.m4a]]\` wikilink |
| \`{{language}}\` | \`ko\` / \`en\` / \`auto\` |
| \`{{model}}\` | \`nova-3\` / \`nova-2\` |
...
2. 설정 → VoxNote → **"Gemini API 키"**에 붙여넣기
...
기존 마크다운 노트 우클릭 → **\`VoxNote ▸ ⭐ AI 요약: 회의록\`** (favorite) 또는 **\`VoxNote ▸ AI 요약 ▸ <템플릿>\`** (그 외). 노트 본문이 입력 transcript가 됩니다 — 다른 템플릿으로 재요약하거나, 이 플러그인이 만들지 않은 노트도 요약 가능.
...
![Rename speaker command](https://raw.githubusercontent.com/Moonjuun/obsidian-voxnote/main/img/img5.webp)
...
| \`{{audio_link}}\` | Wikilink such as \`[[VoxNote/Audio/recording.m4a]]\` |
| \`{{language}}\` | \`ko\` / \`en\` / \`auto\` |
| \`{{model}}\` | \`nova-3\` / \`nova-2\` |
...
2. Settings → VoxNote → **"Gemini API key"** → paste
...
Right-click an audio file → **\`VoxNote ▸\`**:
...
Right-click any markdown note → **\`VoxNote ▸ ⭐ AI summary: Meeting\`** (favorite) or **\`VoxNote ▸ AI summary ▸ <template>\`** (others). The note body becomes the input transcript — useful for trying a different template, or summarizing notes that didn't come from this plugin.
