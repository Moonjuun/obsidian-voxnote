...
회의록의 frontmatter \`speakers\` 항목은 기본적으로 \`화자 0\`, \`화자 1\`... 같은 익명 라벨로 채워집니다. 실제 이름으로 바꿔두면 검색·추적이 쉬워집니다.
...
3. 모달이 열리면 상단에 **현재 노트 이름**이 표시되고, **"변경 전" 칸은 드롭다운**으로 노트에서 자동 감지된 화자 목록이 채워집니다.
   ...
   - **변경 후**: 실제 이름 입력 (예: \`홍길동\`)
4. **치환** 클릭 → 본문의 모든 \`화자 0\`과 frontmatter \`speakers\` 배열의 라벨이 일괄 치환됩니다.
...
| \`{{speakers_list}}\` | 등장 화자 목록을 YAML 배열로 (예: \`["화자 0", "화자 1"]\`) |
...
The \`speakers\` frontmatter starts with anonymous labels (\`화자 0\`, \`화자 1\`, ...). Replacing those with real names makes the notes searchable.
...
3. The modal shows the **current note name** at the top, and the **From field is a dropdown** populated with speakers auto-detected from the note.
   - **From**: pick the speaker to rename (e.g. \`화자 0\`)
...
| \`{{speakers_list}}\` | Detected speakers as YAML inline array (e.g. \`["화자 0", "화자 1"]\`) |
