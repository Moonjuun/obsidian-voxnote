...
1. 오디오 파일 우клик → **`ObsiDeep ▸ ⭐ STT + 요약: Meeting`** (favorite 템플릿은 평면 노출, 나머지는 `AI 요약 ▸` 서브메뉴).
2. 제목 입력.
...
기존 마크다운 노트 우클릭 → **`ObsiDeep ▸ AI 요약: <템플릿>`**. 노트 본문이 입력 transcript가 되고, 새 요약 파일이 `AI-Summaries/`에 추가됩니다. 다른 템플릿으로 같은 회의를 재요약하거나, 이 플러그인이 만들지 않은 노트를 요약할 때 유용합니다.
